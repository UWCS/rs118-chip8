...
/// [`run`][Interpreter::run] starts the interperter, handling sound, windowing, graphics, and timing for you.
/// [`step`][Interpreter::step] should be implemented on a type representing a CHIP-8 Interpreter to run the interpreter one clock cycle at a time, such that [`run`][Interpreter::run] may call it in a loop.
pub trait Interpreter: Sized + Send + 'static {
    ...
    /// Takes ownership of the Interpreter so it is able to  [`run`][Interpreter::run] in an infinite loop.
    fn run(mut self) -> ! {
